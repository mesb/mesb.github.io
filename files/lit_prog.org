...
** A short discourse on Org-mode, literate programming and Reproducible Research
   ...
   executed and their results are added in sections of this
   document. The web page you are viewing is simply an end product of
   a literate programming session. The original draft document that
   bore the pure source code and the explanatory texts were composed
   ...
   it was composed in a kind of file called an /org/ which has the
   ".org" file extension. Emacs automatically recognizes such files as
   Org-mode files and it provides the various facilities that Org-mode
   ...
   (RR)/. Reproducible Research is technique that allows researches to
   ...
   doing, others can follow the steps underlined by the researcher and
   also test their works against other sets of data. Such a practice
   leads to fast implicit peer-reviews and eventually to better
   circulation of scientific works and other works that implement RR.
   ...
   who has access to file, provided they have the powerful tools,
   Emacs and Org-mode.

   Org-mode provides a simple language for describing and typesetting
   documents. A typical *H1* header in Org-mode is  the asterisk(*)
   ...
   of many sorts. Thus, the simple language Org-mode provides is
   sufficient for capturing the literary aspects of /Literate
   ...
   like LaTeX. Actually, LateX is embedded in Org-mode. Now about the
   programming parts.
   ...
   but which harbor source code for computer programs. Such sections
   are called /code blocks/. A good block is specified by the use of
   the keywords: "BEGIN_SRC ..... END_SRC". This block is further
   configured, in a very simple way, in order to let Org-mode know the
   kinds of languages it will deal with and so that it can invoke the
   correct compilers and interpreters. Once program source code is
   written within the /code blocks/, they can be tested and interacted
   with like normal computer programs. The source code can be
   extracted and utilized separately, or the source code and the
   descriptive text or work of literature can be exported to a
   suitable format for circulation or publishing,
...
*** Resources for literate Programming
    ...
    2. [[http://www.literateprogramming.com/knuthweb.pdf][Knuth 1983]]
